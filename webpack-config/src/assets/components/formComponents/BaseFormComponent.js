class BaseComponent {
    ...
    init(option) {
        ...
        for (var i = 0, l = attrs.length; i < l; i++) {
            var attr = attrs[i];
            var name = this.$translate(attr.nodeName);
        ...
        this.isIE = this.$isIE(8);
        this.supportTransition = this.$supportCss3("transition");
     ...
     * 显示验证的错误信息
     */
    addValidateText(text, type = true) {
        //增加显示错误信息格式
        var _this = this;
        _this.hasError = true;
